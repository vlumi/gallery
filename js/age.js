// Copyright 2012 Ville Misaki <ville@misaki.fi>. All rights reserved.
...
function getDateDiffStr(p_startd, p_endd, format, precision) {
	...
	if (format == 'full' || format == 'full_short' || format == 'date' || format == 'date_short') {
		...
		for (var i = 0; i < diffDateParts.length; i++) {
			...
			dateParts.push(format_bignum(val, ',', 0) + title);
		}
		...
		val = format_bignum(val, ',', precision);
...
function format_bignum(num, sep, precision) {
